...
if __name__ == "__main__":
    ...
    fine_tune = False
    model_to_fine_tune = "stead_Smodels_256_0.1/model_S_017.pt"
    if not fine_tune:
        n_epochs = 20
        n_dup = 1
        learning_rate = 0.1
        train_file = "stead_data/validate%s.%ds.%ddup.boxcar.h5"%(phase_type, duration, n_dup)
        model_path = "/home/armstrong/Research/sDetector/stead_%s_models_%s_%s"%(phase_type, batch_size, learning_rate)
    else:
        np.random.seed(2482045)
        n_epochs = 35
        n_dup = 2
        learning_rate = 0.01
        train_file = "uuss_data/train%s.%ds.%ddup.h5"%(phase_type, duration, n_dup)
        model_path = "/home/armstrong/Research/sDetector/uuss_only_%s_models_%s_%s"%(phase_type, batch_size, learning_rate)
