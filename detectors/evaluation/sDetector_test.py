...
import sys
sys.path.append('/home/armstrong/Research/sDetector')
...
if __name__ == "__main__":
    ...
    pref = '/home/armstrong/Research/sDetector'
    model_path = "%s/uuss_%s_models_256_0.01" % (pref, phase_type)

    stead_model = "stead_Smodels_256_0.1/model_S_017.pt"

    #if (test_yellowstone):
    #    model_path = '../../yellowstoneMLPicker/training/python_models/'
    test_type = "validate"
    outpath = model_path + "/test"
    csv_meta = "%s/uuss_data/%s%s.%ds.%ddup.df.csv"%(pref, test_type,  phase_type, duration, n_dup)
    ...
    if (test_yellowstone):
        ...
        csv_meta = "%s/uuss_data/ngb%s.%ds.%ddup.df.csv"%(pref, phase_type, duration, n_dup)
        ...
        csv_meta = "%s/uuss_data/train%s.%ds.%ddup.df.csv"%(pref, phase_type, duration, n_dup)
   ...
   # here
    print("Loading data...")
    if (test_train):
        h5f = "%s/uuss_data/train%s.%ds.%ddup.h5"%(pref, phase_type, duration, n_dup)
        print(h5f)
        ...
        h5f = "%s/uuss_data/%s%s.%ds.%ddup.h5"%(pref, test_type, phase_type, duration, n_dup)
        if (test_yellowstone):
            # NGB.testP.10s.1dup.h5 
            h5f = "%s/uuss_data/ngb%s.%ds.%ddup.h5"%(pref, phase_type, duration, n_dup)
    ...
    #print(df_meta.head())
    #azims = df_meta['back_azimuth_deg'].to_numpy()
    azims = df_meta["source_receiver_azimuth"]
    # azims = None
