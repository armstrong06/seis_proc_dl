...
import sys
sys.path.append('/home/armstrong/Research/onecomp_normalized_noNGB_092013_072014')
from train_gpd_unet_1c import UNet, NumpyDataset
...
def compute_snr(X, Y, T, phase_type = "P", azims = None):
    ...
    for i in range(n_rows):
        ...
        i1 = y_idx[0]
        if (phase_type == "P"):
            x_signal = X[i, y_idx, 0] # Index 2 should be Z
            x_noise = X[i, i0:i1, 0]
...
if __name__ == "__main__":
    ...
    pref = '/home/armstrong/Research/pDetector_onecomp'
    model_path = "%s/%s_models_256_0.001" % (pref, phase_type)

    #if (test_yellowstone):
    #    model_path = '../../yellowstoneMLPicker/training/python_models/'
    test_type = "test"
    outpath = model_path + "/test/proba"
    csv_meta = "%s/data/%s%s.%ds.%ddup.1C.df.csv"%(pref, test_type,  phase_type, duration, n_dup)
    csv_summary = '%s/%s%s.%ds.%ddup.1C.summary.csv'%(outpath, test_type, phase_type, duration, n_dup)
    resid_summary = '%s/%s%s.%ds.%ddup.1C.resid.csv'%(outpath, test_type, phase_type, duration, n_dup)
    
    if save_proba:
        proba_out = '%s/%s%s.%ds.%ddup.1C.proba.h5'%(outpath, test_type, phase_type, duration, n_dup)

    if (test_yellowstone):
        # NGB.testPMeta.10s.1dup.df.csv 
        csv_meta = "%s/data/NGB.test%sMeta.%ds.%ddup.1C.df.csv"%(pref, phase_type, duration, n_dup)
        csv_summary = '%s/test/test%s.NGB.%ds.%ddup.1C.summary.csv'%(model_path, phase_type, duration, n_dup)
        resid_summary = '%s/test/test%s.NGB.%ds.%ddup.1C.resid.csv'%(model_path, phase_type, duration, n_dup)
    
        if save_proba:
            proba_out = '%s/test%s.NGB.%ds.%ddup.1C.proba.h5'%(outpath, phase_type, duration, n_dup)

    elif(test_train):
        csv_meta = "%s/data/train%s.%ds.%ddup.1C.df.csv"%(pref, phase_type, duration, n_dup)
        csv_summary = '%s/train%s.%ds.%ddup.1C.summary.csv'%(outpath, phase_type, duration, n_dup)
        resid_summary = '%s/train%s.%ds.%ddup.1C.resid.csv'%(outpath, phase_type, duration, n_dup)
    
        if save_proba:
            proba_out = '%s/train%s.%ds.%ddup.proba.1C.h5'%(outpath, phase_type, duration, n_dup)
    elif(test_NGBzonly):
        csv_meta = "%s/data/NGB.3C.test%sMeta.%ds.%ddup.df.csv"%(pref, phase_type, duration, n_dup)
        csv_summary = '%s/NGB.3C.Zonly.test%s.%ds.%ddup.summary.csv'%(outpath, phase_type, duration, n_dup)
        resid_summary = '%s/NGB.3C.Zonly.test%s.%ds.%ddup.resid.csv'%(outpath, phase_type, duration, n_dup)

        if save_proba:
            proba_out = '%s/NGB.3C.Zonly.test%s.%ds.%ddup.proba.h5'%(outpath, phase_type, duration, n_dup)
    ...
    # zach_p_model = '/home/jfarrell/MACHINE_LEARNING/model_P_010_0.012348.pt'
    # zach_s_model = '/home/jfarrell/MACHINE_LEARNING/model_S_013_0.015891.pt'

    print("Loading data...")
    if (test_train):
        h5f = "%s/data/train%s.%ds.%ddup.1C.h5"%(pref, phase_type, duration, n_dup)
        print(h5f)
        ...
        h5f = "%s/data/%s%s.%ds.%ddup.1C.h5"%(pref, test_type, phase_type, duration, n_dup)
        if (test_yellowstone):
            # NGB.testP.10s.1dup.h5 
            h5f = "%s/data/NGB.test%s.%ds.%ddup.1C.h5"%(pref, phase_type, duration, n_dup)
        elif (test_NGBzonly):
            h5f = "%s/data/NGB.3C.Zonly.test%s.%ds.%ddup.h5"%(pref, phase_type, duration, n_dup)
    ...
    metrics = []
    resids = []
    unet = UNet(num_channels=1, num_classes=1).to(device)
    ...
    for epoch in range(n_epochs):
        print("Testing:", epoch)
        model_in = os.path.join(model_path, models_to_test[epoch])
        epoch=32
